...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    int i = 0;
    for (; i < static_cast<int>(key.size()) - 1; i++) {
      char c = key[i];
      auto next = p->get()->GetChildNode(c);
      if (!next) {
        next = p->get()->InsertChildNode(c, std::make_unique<TrieNode>(c));
      }
      p = next;
    }
    bool ret = false;
    char c = key[i];
    auto end = p->get()->GetChildNode(c);
    if (end) {
      if (!end->get()->IsEndNode()) {
        auto new_end = new TrieNodeWithValue<T>(std::move(*(*end)), value);
        end->reset(new_end);
        ret = true;
      }
    } else {
      p->get()->InsertChildNode(c, std::make_unique<TrieNodeWithValue<T>>(c, value));
      ret = true;
    }
    latch_.WUnlock();
    return ret;
   ...
   */
  bool RemoveHelper(std::unique_ptr<TrieNode> *p, const std::string &key, int idx) {
    ...
    bool ret = RemoveHelper(next, key, idx + 1);
    if (ret && !next->get()->IsEndNode() && !next->get()->HasChildren()) {
      p->get()->RemoveChildNode(c);
    }
    return ret;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.WLock();
    bool ret = RemoveHelper(&root_, key, 0);
    latch_.WUnlock();
    return ret;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto last_node = dynamic_cast<TrieNodeWithValue<T> *>(p->get());
    if (p->get()->IsEndNode() && last_node != nullptr) {
      *success = true;
    }
    latch_.RUnlock();
    if (*success) {
